...
nocite: |
  @huynh2010inferring, @chang2020mirnet, @robinson2010edger, @csardi2006igraph, @villanueva2019ggplot2, @love2014moderated, @genomictools, @lawrence2013software, @adar2023integrate, @dplyr, @stuart2011matchit, @networkd3, @ahmed2021maternal, @yu2015chipseeker, @machlab2022monalisa, @li2020compsra
---
...
10. ***plotATACMotifHeatmap*** for visualizing the motif enrichment analysis
    as a heatmap comparing differentially enriched motifs between the two
    testing conditions
11. ***matchSamplesRNAsmallRNA*** for matching the samples between RNAseq and
    small RNAseq data
12. ***exportMatchResult*** for exporting the matching results to a data frame
13. ***loadExtInteractions*** for loading external interaction data for 
    small RNA - gene and TF - gene interactions
14. ***setOmicCutoffs*** for setting the cutoffs for differential expression
...
![Overview of the package analysis pipeline](./inst/extdata/Schematics.jpg)


## Contributions

The author of the package is Jielin Yang. The author defined all data structures
used in this package, including the S4 classes `MOList`, `DETag`, `TOPTag`, `PEAKTag`, and `TRNet`. The author wrote the `MOList` function to construct the
